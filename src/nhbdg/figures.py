...
def S(f,o,n):
    plt.rcParams.update({"font.size":8,"axes.labelsize":8,"legend.fontsize":6,"pdf.fonttype":42});
    for i,a in enumerate(f.axes): a.text(-.14,1.05,f"({chr(97+i)})",transform=a.transAxes,fontweight="bold"); h,l=a.get_legend_handles_labels(); a.legend(frameon=False) if h else None
...
def figure02(d,o):
 ...
 for i,col in enumerate(["delta_plus_abs","delta_minus_abs"]):
  z=p[p.kind=="mapped"];a[0,0].semilogy(z.j,z[col],color=C[i],label="mapped"+col);[a[0,0].semilogy(q.j,q[col],ls="None",marker=m,markevery=5,mfc="white",color=C[i],label=k+col) for k,m in [("raw","o"),("rescaled","s")] for q in [p[p.kind==k]]]
 z=p[p.kind=="mapped"];a[0,1].plot(z.j,z.P_real,label=r"$P_j/t^2$");a[0,1].plot(z.j,z.delta_plus_abs*z.delta_minus_abs,"--",color=".3",label=r"$|\tilde\Delta|^2/t^2$");a[1,0].plot(z.j,z.density,label="OBC")
 ...
 for q in a.flat:q.set_xlabel("site $j$")
...
def figure03(d,o):
 ...
 z=t[(t.study=="fig3")&np.isclose(t.g,.05)&t.quantity.isin(["metric_1e-2","pair","gamma"])]
...
def figure_s3(d,o):
 x=V(pd.concat([R(d/"fig3.csv"),R(d/"fig4.csv")]));f,a=plt.subplots(1,4,figsize=(7.2,2.5));cols=["s_min","minimum_eigenvalue_separation","right_condition","field_residual"]
 for i,(k,q) in enumerate(x.groupby(["study","L","g"])):
